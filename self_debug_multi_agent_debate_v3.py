...
from chat import ai_critic_word, chat_selfdebug_debate, chat_merge_debug_results,choose_better_result,final_evaluate,chat_selfdebug
...
def debate(individual_results: List[str],
           buggy_code: str,
           test_case: List[str],
           task_description: str) -> str:
    """
    多轮两两对比辩论，选出最优修复代码
    individual_results: [json.dumps(basic_debug_result)] 的数组
    buggy_code: 原始bug代码
    test_case: 测试用例字符串
    task_description: 任务描述
    choose_better_result: 函数 choose_better_result(buggy_code, result1, result2, task_description, test_case)
                          -> JSON字符串，包含 "better_result_index"、"final_corrected_code" 等
    final_evaluate: 最后评估函数，对 top3 候选进行综合评估并返回最终结果
    """
    ...
    for idx, res in enumerate(individual_results):
        try:
            data = json.loads(res)
            data["index"] = idx
            data["support"] = 0  # 初始化支持率
            candidates.append(data)
        except Exception as e:
            print(f"Error parsing result {idx}: {e}")

    print(f"[Init] 已载入 {len(candidates)} 个候选")

    # Step 2: 两两辩论循环
    round_num = 1
    while len(candidates) > 3:  # 直到剩下3个候选
        ...
        for i in range(0, len(candidates) - 1, 2):
            ...
            print(f"\n--- 比较 候选{r1['index']} vs 候选{r2['index']} ---")

            code1= r1["corrected_code"]
            code2= r2["corrected_code"]
            if code1.strip() == code2.strip():
                print(f"两个候选代码相同，直接晋级 候选{r1['index']}")
                r1["support"] += 1
                next_round_candidates.append(r1)
                continue

            debate_result = choose_better_result(
                buggy_code=buggy_code,
                result1=json.dumps(r1, ensure_ascii=False),
                result2=json.dumps(r2, ensure_ascii=False),
                task_description=task_description,
                test_case=test_case
            )
            if debate_result == "":
                print("choose_better_result 返回空结果，跳过此次比较")
                next_round_candidates.append(r1)
                next_round_candidates.append(r2)
                continue
            try:
                result_json = json.loads(debate_result)
            except Exception as e:
                print(f"解析 choose_better_result 输出失败: {e}")
                continue

            better_idx = result_json.get("better_result_index", "0")
            final_code = result_json.get("final_corrected_code", "")
            analysis = result_json.get("analysis", "")
            rationale = result_json.get("selection_rationale", "")

            print(f"模型裁决: better={better_idx}, 理由={rationale}")
            ...
            if better_idx == "1":
                ...
                # 如果两个都不对，模型生成新版本
                new_candidate = {
                    "index": f"gen_{round_num}_{i}",
                    "test_case": test_case,
                    "bug_analysis": "LLM-generated optimal alternative",
                    "corrected_code": final_code,
                    "explanation": rationale,
                    "support": 1
                }
                print(f"→ 模型生成了新的修复方案 {new_candidate['index']}")
                next_round_candidates.append(new_candidate)

        # 若为奇数个，保留最后一个直接晋级
        if len(candidates) % 2 == 1:
            next_round_candidates.append(candidates[-1])
    ...
    print("\n=== 最终进入评估环节的候选 ===")
    for c in top3:
        print(f"候选 {c['index']}，支持率 {c['support']}")
...
def process_single_task_multi(task_data: dict, task_index: int, timeout: int = 30002) -> Tuple[str, bool, Optional[bool], str, str, bool]:
    """
    使用多轮调试方法处理单个任务
    返回: (task_id, debug_success, test_passed, error_type, details, has_test_cases)
    """
    # if task_index != 41:
    #     return "", False, None, "skipped", "Skipped non-6 task", False
    ...
    try:
        ...
        if not test_cases:
            ...
            try:
                ...
                write_str_to_file(final_corrected_code, temp_filename)
                textcfg = TextCFG(temp_filename, func_name)
                cfg_text = textcfg.cfg_text  # 每次重新获取最新CFG
            ...
            for i, test_case in enumerate(test_cases, 1):
                # 检查超时
                elapsed_time = time.time() - task_start_time
                if elapsed_time > timeout - 1200:  # 留20分钟给合并
                ...
                try:
                    ...
                    for j in range(3):
                        ...
                        analyse = full_debug_json.get("correctedCode_test_analysis", {})
                        final_corrected_code = analyse.get("corrected_code", buggy_code)
                        final_corrected_code = ExpressionFlattener().flatten_code(final_corrected_code)
                        write_str_to_file(final_corrected_code, temp_filename)
                        textcfg = TextCFG(temp_filename, func_name)
                        cfg_text = textcfg.cfg_text  # 每次重新获取最新CFG
                        isPassed,runDetails= ai_critic_word(task_description,cfg_text, test_case, final_corrected_code)
                        if not isPassed:
                    ...
                    try:
                        full_debug_json = json.loads(full_debug_result)
                        analyse = full_debug_json.get("correctedCode_test_analysis", "No analysis provided")
                        case_details =analyse.get("test_cases", "")
                        corrected_code = analyse.get("corrected_code", buggy_code)
                        explanation = full_debug_json.get("explanation", "No explanation provided")
                        overall_analysis = full_debug_json.get("overall_analysis", {})
                        write_str_to_file(corrected_code, temp_filename)
                        textcfg = TextCFG(temp_filename, func_name)
                        cfg_text = textcfg.cfg_text  # 每次重新获取最新CFG
                        simplified_result = {
            ...
            try:
                #选出
                final_corrected_code = debate(
                    individual_results=individual_results,
                    buggy_code=buggy_code,
                    test_case=test_cases,
                    task_description=task_description
                )
                final_result_json = json.loads(final_corrected_code)
                final_corrected_code = final_result_json.get("final_corrected_code", final_corrected_code)
                safe_print(f"[{task_index}] Debate completed for {task_id}, selected final correction")
            except Exception as e:
                safe_print(f"[{task_index}] Debate failed for {task_id}: {str(e)[:100]}...")
                return task_id, False, None, "debug_error", f"Debate error: {str(e)}", has_test_cases
        # 检查是否获得了修正代码
...
def main():
    ...
    task_timeout = 30000  # 每个任务30秒超时（多轮调试需要更多时间）
    overall_timeout = 360000  # 总体600分钟超时
            ...
            try:
                for future in as_completed(future_to_index, timeout=overall_timeout):
                    ...
                    try:
                        ...
                        safe_print(f"Task {task_index} result timeout after {task_timeout + 600}s")
                        results.add_result(f"task_{task_index}", False, None, "timeout_error", 
                                         f"Task result timeout after {task_timeout + 600}s", False)
                        completed_futures.append(future)
                    except Exception as e:
    ...
    output_file = f"./dataset_test/humanevalfix/results/multi_debug_results_debate_{timestamp}.json"
