...
from chat import ai_critic_word, chat_selfdebug_debate, chat_merge_debug_results,choose_better_result,final_evaluate
...
def process_single_task_multi(task_data: dict, task_index: int, timeout: int = 30002) -> Tuple[str, bool, Optional[bool], str, str, bool]:
    ...
    """
    if task_index != 54:
        return "", False, None, "skipped", "Skipped non-54 task", False
        ...
        if not test_cases:
            ...
            try:
                for i in range(3):
                    ...
                    write_str_to_file(final_corrected_code, f"debug_no_testcase_{task_id}_{i}.py")
                    textcfg = TextCFG(f"debug_no_testcase_{task_id}_{i}.py", func_name)
                    cfg_text = textcfg.cfg_text
                    isPassed,runDetails= ai_critic_word(task_description,cfg_text, "No test cases available", final_corrected_code)
                    if not isPassed:
            ...
            for i, test_case in enumerate(test_cases, 1):
                ...
                try:
                    ...
                    full_debug_result  = chat_selfdebug_debate(
                        buggy_code=buggy_code,
